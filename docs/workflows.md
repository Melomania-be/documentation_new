...
> If port 5432 is already in use on your machine, use DB_PORT=5433 instead and update the docker-compose.yml ports accordingly.
...
> Emails don't actually send locally but these values must exist or AdonisJS will block startup.
...
> node ace db:seed creates the admin user using the ADMIN_EMAIL and ADMIN_PASSWORD values from your .env. If you skip this step, the admin account will not exist yet.
